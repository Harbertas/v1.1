...
double mediana(duomenys& temp){
    ...
    deque<int>::iterator it = temp.paz.begin();
    deque<int>::iterator it2 = temp.paz.begin();
    if(temp.kiekPaz % 2 != 0){
        // 1 2 3
...
void spausdinti(deque<duomenys> &sarasas, deque<duomenys> &sarasas2, int fileLength/*, double &timeTaken*/){
...
void rikiavimas(deque<duomenys>& sarasas, int fileLength, double &timeTaken){
    string rusiuoti = "3";
...
void generateFileData(string generatedFileName, int fileLength, int pazymiuKiekis, double & timeTaken){
    deque<string> vardas = {"Petras", "Antanas", "Jonas", "Tomas", "Valdas", "Romas", "Lukas", "Matas", "Virgis", "Mantas", "Kazys"};
    string generatedName;

    deque<string> pavarde = {"Petraitis", "Antanaitis", "Jonaitis", "Tomaitis", "Valdaitis", "Romaitis", "Lukaitis", "Mataitis", "Virgaitis", "Mantaitis", "Kazaitis"};
...
void atskirti(deque<duomenys>& sarasas, deque<duomenys>& sarasas2, int fileLength, double &timeTaken){
    auto start = std::chrono::high_resolution_clock::now();

//    deque<duomenys>::iterator it;
...
//    sarasas.erase(it, sarasas.end());
    ...
    deque<duomenys>::reverse_iterator it;
